...
class ClothingRemoteMediator (
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Clothing>): MediatorResult {
        return try {
            ...
            val response = clothingApi.getAllClothing(page = page)
            if (response.clothing.isNotEmpty()) {
                clothingDatabase.withTransaction {
                    ...
                    val keys = response.clothing.map { clothing ->
                        ClothingRemoteKeys(
                            id = clothing.id,
                            prevPage = prevPage,
                            nextPage = nextPage,
                    ...
                    clothingDao.addClothing(clothing = response.clothing)
                }
            }
